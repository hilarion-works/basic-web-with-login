...
export const auth = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (req.header('Authorization') !== undefined && req.header('Authorization') !== null) {
      ...
      if (!user) return response.invalidInput('No User Found from this token id', res, 301);
      // req.user = user.dataValues
    } else {
      return response.unauthorized('Unauthorized access. Please contact our technical support.',res, 400);
    }
  } catch (error) {
    console.log("err try: ", error.message)
    return response.error('Internal server error, please try again in a couple minutes.', res, 500);
  }
  next();
};
